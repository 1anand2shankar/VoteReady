/**
 * @module AIAssistant
 * @description VoteReady — 3-Level Resilient AI Architecture.
...
function resetUsage() {
  const d = {
    date: new Date().toDateString(), geminiReq: 0, mistralReq: 0, geminiFail: 0, mistralFail: 0,
...
// ── System Prompt ──
const SYS = `You are VoteReady — India's election education assistant. Rules:
...
async function tryMistral(body) {
  try {
    ...
    if (!res.ok) {
      ...
      console.warn('[VoteReady] Mistral HTTP', res.status, errBody.slice(0, 200));
      return null;
    }
    const data = await res.json();
    ...
    console.warn('[VoteReady] Mistral error:', e.message);
  }
  return null;
}
...
export async function askGemini(question) {
  ...
  const body = {
    contents: [
      ...
      { role: 'model', parts: [{ text: 'I am VoteReady, an apolitical election education assistant for India. How can I help?' }] },
      ...chatHistory
    ],
